...
Learn.startLearning = function () {
  ...
  while (Learn.genomes.length < Learn.genomeUnits) {
    Learn.genomes.push(Learn.buildGenome(4, 1));
  }
...
Learn.executeGeneration = function (){
  ...
  async.mapSeries(Learn.genomes, Learn.executeGenome, function (argument) {
    ...
    for (var i = Learn.genomes.length-1; i >= 0; i -= 2) {
        var first = Learn.selectBestGenomes()[0].toJSON();
        var second = Learn.selectBestGenomes()[0].toJSON();
        
        var newfirst = Learn.crossOver(first, second);
        var newsecond = Learn.crossOver(second, first);
        
        first = Learn.mutate(newfirst);
        second = Learn.mutate(newsecond);
        
        Learn.genomes.push(Network.fromJSON(first));
        Learn.genomes.push(Network.fromJSON(second));
    }
    // Execute next generation
    Learn.executeGeneration();
    /*
    // Kill worst genomes
    Learn.genomes = Learn.selectBestGenomes();
...
// Returns randomly selected element from list but genomes with higher fitness
// have a higher chance of being selected.
Learn.selectBestGenomes = function (){
  var total = 0.0;
  _.forEach(Learn.genomes, function(value) {
    total += value.fitness;
  });
  var randTotal = total * Math.random();
  var newTotal = 0.0;
  for (var i = Learn.genomes.length - 1; i >= 0; i--) {
    var thing = Learn.genomes[i];
    newTotal += thing.fitness;
    if (newTotal >= randTotal) {
        return Learn.genomes.splice(i, 1);
    }    
  }
  return Learn.genomes.splice(Learn.genomes.length - 1, 1);
  /*
...
Learn.executeGenome = function (genome, next){
  ...
  var num = Learn.genomes.indexOf(genome) + 1;
  Learn.genome = num;
  ...
  while (true) {
    ...
    if (obstacles.length > 0) {
        //Read 3 inputs, apply network until game ends
        var isFlying = (obstacles[0].typeConfig.type === 'PTERODACTYL') ? 1 : 0;
        inputs = [
            (obstacles[0].xPos) - (tRex.config.WIDTH + tRex.xPos),
            obstacles[0].size,
...
},{"async":3,"lodash":4,"synaptic":9}],3:[function(require,module,exports){
...
},{"./layer":6,"./network":7,"./trainer":10}],6:[function(require,module,exports){
// export
...
},{"./layer":6,"./neuron":8,"./trainer":10}],8:[function(require,module,exports){
// export
...
},{"./architect":5,"./layer":6,"./network":7,"./neuron":8,"./trainer":10}],10:[function(require,module,exports){
// export
